...
import '../final_resources_screen.dart';
import '../model/sub_cate_model.dart';
import '../summary3bloc/summary3_bloc.dart';

class SubCategory2Widget extends StatefulWidget {
  final String? rootId;
  final Color? color;
  final String? categoryName;
  final int ? level;

   SubCategory2Widget({required this.rootId, required this.color, required this.categoryName, required this.level});
  @override
  State<SubCategory2Widget> createState() => _SubCategory2WidgetState();
}

class _SubCategory2WidgetState extends State<SubCategory2Widget> {
  ...
  void initState() {
    ...
    context.read<SubCategory2Bloc>().add(
        SubCategory2LoadEvent(rootId: widget.rootId));
  ...
  addCategory({required String summary}) async {
    ...
    if (res.statusCode == 200) {
      ...
      context.read<Summary3Bloc>().add(Summary3LoadedEvent(rootId: widget.rootId));
      setState(() {
  ...
  AwesomeDialog DeleteCategory({
    ...
    required List<SubCategory2Model> cateList,
    required int index
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      floatingActionButton: ElevatedButton(
        ...
        onPressed: (){
          Navigator.push(context, MaterialPageRoute(builder: (context)=>CreateSubCate2Screen(rootId: widget.rootId,
      ...
      Column(
        children: [
          ...
          SizedBox(
            ...
            child: BlocBuilder<Summary3Bloc, Summary3State>(builder: (context, state){
              if(state is Summary3Loading){
                return SizedBox();
                //   Center(
                //   child: Container(
                //     height: 50,
              ...
              if(state is Summary3LoadedState){
                summary.clear();
          ...
          Expanded(
            child: BlocBuilder<SubCategory2Bloc, SubCategory2State>(
