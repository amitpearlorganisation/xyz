...
class AddPromptsAddResourceScreen extends StatefulWidget {
  ...
  const AddPromptsAddResourceScreen({required this.resourceId, required this.categoryId, Key? key})
      : super(key: key);

  @override
  State<AddPromptsAddResourceScreen> createState() => _AddPromptsAddResourceScreenState();
}

List<String> mediaType = ['Audio', 'Text', 'Video', 'Image'];

class _AddPromptsAddResourceScreenState extends State<AddPromptsAddResourceScreen> {
  ...
  Future<void> onAddPromptPressed(AddPromptsInitial state, BuildContext context) async{
    if (!_titleformKey.currentState!.validate()) {
      ...
      context.read<AddPromptsBloc>().add(
        AddPromptEventforQuickPrompt(
            // resourceId: widget.resourceId!,
            // categoryId: widget.categoryId!,
            Promptname: title_Controller.text),
      );
